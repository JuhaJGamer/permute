...
std::string permute(std::string text, unsigned int seed) {
    std::vector<std::string> blocks;
    std::default_random_engine e1(seed);
    for(int i = 0; i < text.size();) {
        std::poisson_distribution<int> uniform_dist(4>text.size()-i?text.size()-i:4);
        int rand = -1;
...
int main(int argc, char** argv) {
    ...
    if(positional.size() == 0) {
        ...
        std::cout << permute(text, seed) << "\n";
    }

    if(STDIN) {
        while(! std::cin.eof()) {
            std::getline(std::cin,text);
            std::cout << permute(text, seed) << ((text.size() == 0)? "":"\n");
            text = "";
        }
    }
}
